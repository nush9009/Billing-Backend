...
Revision ID: 66f354964c54
Revises: 
Create Date: 2025-09-11 16:22:19.866464
...
def upgrade():
    ...
    with op.batch_alter_table('project_billing', schema=None) as batch_op:
        ...
        batch_op.alter_column('status',
               existing_type=sa.VARCHAR(length=20),
               nullable=False)
        batch_op.drop_constraint(batch_op.f('project_billing_pricing_id_fkey'), type_='foreignkey')
        batch_op.drop_column('milestone_description')
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('project_billing', schema=None) as batch_op:
        batch_op.add_column(sa.Column('pricing_id', sa.VARCHAR(length=36), autoincrement=False, nullable=True))
        batch_op.add_column(sa.Column('paid_date', sa.DATE(), autoincrement=False, nullable=True))
        batch_op.add_column(sa.Column('hours_worked', sa.NUMERIC(precision=8, scale=2), autoincrement=False, nullable=True))
        batch_op.add_column(sa.Column('invoice_date', sa.DATE(), autoincrement=False, nullable=True))
        batch_op.add_column(sa.Column('milestone_description', sa.TEXT(), autoincrement=False, nullable=True))
        batch_op.create_foreign_key(batch_op.f('project_billing_pricing_id_fkey'), 'project_pricing', ['pricing_id'], ['id'])
        batch_op.alter_column('status',
               existing_type=sa.VARCHAR(length=20),
               nullable=True)
